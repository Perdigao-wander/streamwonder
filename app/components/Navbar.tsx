...
import { Menu, X, PlayCircle, BookOpen, Film, Tv, Home, Drama, JapaneseYen, Antenna  } from 'lucide-react';
...
const Navbar = () => {
    ...
    const navLinks = [
        ...
        { href: '/canais', label: 'Canais', icon: Antenna },
    ...
    return (
        <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
                    ...
                    <div className="hidden md:flex items-center gap-1">
                        {navLinks.map((link) => (
                            <Link
                                key={link.label}
                                href={link.href}
                                className="relative px-4 py-2 rounded-full transition-all duration-300 group text-gray-300 hover:text-white"
                            >
                                <span className="relative z-10 flex items-center gap-2">
                                    <link.icon className="w-4 h-4" />
                                    <span className="font-medium text-sm">{link.label}</span>
                                </span>
                                <span className="absolute inset-0 bg-white/0 group-hover:bg-white/10 rounded-full transition-all duration-300"></span>
                            </Link>
                        ))}
                    </div>
            ...
            {/* Mobile Menu - Desliza de cima (igual ao exemplo) */}
